...
Now equipped with passive background auto-refresh via @st.fragment (no full-page reloads).
"""
...
# -----------------------------------------------------------------------------
# 3. SIDEBAR CONTROLS & PASSIVE REFRESH SETTINGS
# -----------------------------------------------------------------------------
with st.sidebar:
    ...
    # Passive auto-refresh selector
    auto_refresh = st.selectbox(
        "Passive Auto-Refresh",
        ["Off", "10 seconds", "30 seconds", "60 seconds"],
        index=1,
        help="Silently updates data, positions, and charts in the background without refreshing the browser page or resetting your view."
    )
...
# -----------------------------------------------------------------------------
# 4. LIVE DASHBOARD FRAGMENT (PASSIVE REAL-TIME UPDATES)
# -----------------------------------------------------------------------------
@st.fragment(run_every=f"{refresh_seconds}s" if refresh_seconds > 0 else None)
def render_live_dashboard(tracking_mode: str, dust_threshold: float, refresh_label: str, refresh_secs: int):
    """
    Renders the live trading terminal within a background Streamlit fragment.
    Updates in place via WebSockets with zero page flicker, no white screen, and no scroll jumps!
    """
    ...
    # 2. FINANCIAL CALCULATIONS & "3-BULLET" METRICS
    ...
    # 3. HEADER & TOP KPI METRICS (ROW 1)
    ...
    with header_col2:
        ...
        refresh_pill = f'<span class="status-pill status-pill-online">⚡ Passive Sync ({refresh_label})</span>' if refresh_secs > 0 else '<span class="status-pill status-pill-offline">⏸️ Passive Sync Off</span>'
        ...
        st.markdown(
            ...
            <div style="font-size: 0.78rem; color: #94a3b8; margin-top: 6px;">
                {market_subtext} &bull; <span style='color: #64748b;'>Updated: {datetime.now().strftime('%H:%M:%S')}</span>
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    # 4. ACTIVE POSITIONS & 3-BULLET SLOTS (ROW 2)
    ...
    # 5. INTERACTIVE CHARTS & CAPITAL ALLOCATION (ROW 3)
    ...
    # 6. TRADE HISTORY & PERFORMANCE LOG (ROW 4)
    ...
    # 7. OPEN ORDERS & SYSTEM AUDIT FEED (ROW 5)
...
# -----------------------------------------------------------------------------
render_live_dashboard(tracking_mode, dust_threshold, auto_refresh, refresh_seconds)
